...
public class Adapter_set_address extends BaseAdapter{
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(position == list.size() - 1){
            holder.iv_setImg.setBackgroundDrawable(context.getResources().getDrawable(R.mipmap.rep_btn_add));
        }else{

            if ((Integer)list.get(position).get(flag[3]) == 1) {
                holder.iv_setImg.setVisibility(View.VISIBLE);
                holder.iv_setImg.setBackgroundDrawable(context.getResources().getDrawable(R.mipmap.col_btn_sel_1));
            } else {
